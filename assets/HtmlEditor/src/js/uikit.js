...
(function(core) {
    ...
    if (!window.jQuery) {
        throw new Error('uikit2 2.x requires jQuery');
    } else {
        uikit2 = core(window.jQuery);
    }

    if (typeof define == 'function' && define.amd) { // AMD

        define('uikit2', function(){

            uikit2.load = function(res, req, onload, config) {

                var resources = res.split(','), load = [], i, base = (config.config && config.config.uikit2 && config.config.uikit2.base ? config.config.uikit2.base : '').replace(/\/+$/g, '');

                if (!base) {
                    throw new Error('Please define base path to uikit2 in the requirejs config.');
                }
